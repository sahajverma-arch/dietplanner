  ...
  const CAT_ROWS = [
    {
      ...
      calories: "TDEE − 17.5% (midpoint of the 15–20% band). If current intake exceeds the target by more than 400 kcal, weeks 1–2 run at the midpoint of the two and the full target starts week 3.",
      protein: "1.35 g/kg",
      sec: "§6.1, §6.2, §7.2",
    },
    {
      ...
      calories: "Three adaptation tests. If any fires: diet break at TDEE for 10–14 days (extra energy to carbohydrate, +1,500–2,000 steps), then re-enter at −20%. If none fires: hold the target and audit 14 days of weighed logging.",
      protein: "1.9 g/kg",
      sec: "§6.4, §7.2",
    },
    {
      ...
      calories: "Ramps −10% → −15% → −20% across weeks 1–3, then holds. Each step is ~5% of TDEE, below the level at which a change is consciously felt. Progression is gated on behaviour (6 of 7 days logged), never on the scale.",
      protein: "1.9 g/kg",
      sec: "§6.5, §7.2",
    },
    {
      ...
      calories: "Eat at TDEE. If current intake is more than 300 kcal below it, reverse-diet at +125 kcal per week until TDEE is reached — a step small enough to stay under the scale's noise floor.",
      protein: "1.5 g/kg",
      sec: "§6.6, §7.2",
    },
  ];
    ...
    <Document title="LEANR diet engine — how the roadmap works">
      ...
      <Page size="A4" style={s.page}>
        ...
        <Text style={s.p}>
          ...
          intake below BMR means real adaptation or substantial under-reporting; both mean stop
          cutting.{" "}
        ...
        <Text style={s.p}>
          ...
          <Text style={{ fontFamily: "Helvetica-Bold" }}>§10.6</Text> A Category 1 client already
          eating below the computed target is warned, not silently told to eat more.{"  "}
      ...
      <Page size="A4" style={s.page}>
        ...
        <Text style={[s.p, s.small]}>
          She eats {intake.kcalPerDay} kcal now, which is {intake.kcalPerDay - priya.targetKcal} kcal
          against the target — inside normal daily variation, so no two-week transition phase is
          created and week 1 starts at the full figure. Because she is already at or below it, the
          engine raises the §10.6 warning rather than issuing an instruction to eat more.
        </Text>
        ...
        <Text style={[s.p, s.small, { marginTop: 4 }]}>
          Flat, because no transition phase applies to her. A Category 3 re-starter would instead
          step down each week, and a first-timer whose intake exceeded the target by more than
          400 kcal would run two weeks at the midpoint first.
        </Text>

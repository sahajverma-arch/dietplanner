...
18. MUSCLE, TRAINING AND RECOVERY: consider muscle preservation, training performance and recovery. DISTRIBUTE the day's protein figure across the meals — distribute, not add to: the daily total is fixed by rule 8, and spreading it evenly across breakfast, lunch and dinner is the goal, not stacking a protein source into every meal until the total runs past the target. A vegetarian day of dal at every meal PLUS curd PLUS paneer PLUS tofu overshoots by 30-40 g; pick the protein foods that reach the figure and stop there. Use sources this client actually accepts, respecting the protein barriers. VERIFIED PROTEIN (from the same database that re-costs this plan — plan by these numbers, not by intuition): ${PROTEIN_REFERENCE.map(
...
function prescriptionBlock(ctx: PlanContext): string {
  ...
  const lines = [
    ...
    // A target alone reads as "at least". Left with only a figure to aim at,
    // the model built an Indian vegetarian week to 76 g against a 55 g target,
    // because roti + dal + curd + paneer lands there on its own and nothing
    // told it that was a miss. So the ceiling is stated as a number, and the
    // per-meal split is done here rather than left as an exercise — "55 g a
    // day" is not actionable while composing a single meal.
    `- PROTEIN CEILING: no day may exceed ${proteinCeiling(t.protein_g)} g. A day above that has MISSED the target, exactly as a day below ${Math.max(0, t.protein_g - 5)} g has. Aim at ${t.protein_g} g, not above it.`,
    `- To land there: roughly ${perMainMeal(t.protein_g)} g of protein at each of the three main meals and ${perSnack(t.protein_g)} g at each snack. Do NOT put a concentrated protein food (paneer, tofu, soya, eggs, chicken, curd) in every single meal on top of dal — dal and roti already carry ${perSnack(t.protein_g)}-${perMainMeal(t.protein_g)} g between them, and stacking another source on top is what overshoots.`,
    `- Fat: ${t.fat_g} g`,
    `- Carbohydrate: ${t.carbs_g} g. This is high on purpose — it is what the calories are for once protein is capped. Fill the plate with vegetables, roti, rice and fruit, not more protein.`,
